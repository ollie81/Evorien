...
Ollieen is the digital foundation for a future network of high-autonomy
...
- A free **Supabase** account — <https://supabase.com>. Supabase is the
  database and login system behind Ollieen.
...
2. Click **New project**.
3. Give it a name (e.g. "Ollieen"), set a database password (save it
...
Ollieen's entire database structure lives in `supabase/migrations/` as plain
...
This creates every table, security rule, and the Passport numbering
system. Nothing here contains real user data — it's just structure.

> **Naming note:** Passport codes look like `EVR-000001` — that prefix is
> generated by the database (not this app's code) and predates the
> Ollieen name. It's cosmetic only, doesn't affect anything functionally,
> and changing it now would require a schema migration touching a
> generated column on a real, live table — out of scope for a branding
> update. See part C of the migration report for more on this.
...
Ollieen uses Supabase Auth directly for everything — email/password and
...
sign-in link to land on an error page instead of back on Ollieen.

**Site URL and Redirect URLs** (required for email confirmation links,
password reset links, and Google sign-in to return to Ollieen correctly —
...
2. Under **Authorized redirect URIs**, add your Supabase project's own
   ...
   Supabase first regardless of where Ollieen itself is running.
...
2. Complete onboarding — this generates your Passport
...
5. Make sure that same URL is in **both** places in Supabase — see
   **Configure authentication** under step 2 above:
   ...
   `localhost`, instead of returning to Ollieen: Supabase falls back to
...
## 7. Connect ollieen.com

Ollieen is fully live at your Vercel-assigned URL from step 6 (e.g.
`your-app.vercel.app`) — nothing below is required for the site to work.
This is what to do when you're ready to put `ollieen.com` in front of it,
without breaking the working deployment while you do.

1. In Vercel, open your project -> **Settings -> Domains** -> add
   `ollieen.com` (and `www.ollieen.com` if you want both). Vercel shows
   the exact DNS records to add at your domain registrar.

   > If `evorien.com` is still listed here from an earlier attempt to
   > connect that domain, it's harmless to leave (it was never verified,
   > so nothing depends on it) — remove it whenever convenient, or just
   > ignore it.
...
3. **Only once `ollieen.com` shows Valid in Vercel**, finish the cutover:
   ...
   - Keep the existing `your-app.vercel.app` entry in Redirect URLs, and
     ...
     `ollieen.com` — the old URL keeps working as a fallback the whole
     time you're verifying the new one.
4. Optional polish once `ollieen.com` is live: add it to the **Authorized
...
Ollieen AI is an intelligence layer over everything above — it helps a
...
**Where this stands right now:** there's a real "Ask Ollieen AI" screen
...
1. Get an OpenAI API key, if you don't already have one: go to
   ...
   **API keys**, and create a new key. You'll also need billing set up on
   that OpenAI account — Ollieen AI calls cost a small amount per message.
2. In Vercel, open your project -> **Settings -> Environment Variables**
   ...
   - `AI_DAILY_MESSAGE_LIMIT` — optional. Leave unset for the default of
     30 Ollieen AI messages per member per day. Every founding member gets
...
### What Ollieen AI can look up (and how it stays scoped)
...
- It never speaks for Ollieen's governance or tells anyone how to vote.
- It never claims Ollieen owns land, controls territory, or operates a
...
app/
  ...
  api/ai/           Ollieen AI's Route Handlers — the app's only API routes.
...
lib/
  ...
  ai/               Ollieen AI: model.ts (OpenAI wrapper, reads
  ...
  skills.ts         getOrCreateSkillId() — shared by Passport skills,
                    project skills, and Ollieen AI's project builder so
...
your own Passport, a "For You" tab that ranks real members by compatibility
with your own Passport (shared pillars/skills, an open need on one of your
own projects, or a role matching what you're looking for), and a complete
Build ecosystem: project creation, a public project
...
network view on the Passport), a read-only view of another member's
Passport, a public `/welcome` vision page that
...
chain, date/time formatting, slug generation, collaborator-match scoring.
...
**Ollieen AI:** an ongoing, staged build (see section 8 above) — a real
"Ask Ollieen AI" chat (linked from Home, at `/ai`) grounded in ten
...
Ollieen. Beyond direct search, `match_contributions` proactively connects
...
`ai_messages`). "Ask Ollieen AI" now also appears contextually on Home,
...
Ollieen AI, governance) stays free forever, with no feature ever taken
...
paid access). The one real, working example: Ollieen AI's daily message
